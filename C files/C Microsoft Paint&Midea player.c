...
static void open_mp4(GtkWidget *widget, gpointer data);
...
static void save_history(Paint *app) {
    if (app->history_count < 100) {
        // 기존 900, 600에서 1919, 1035로 변경!
		cairo_surface_t *copy = cairo_image_surface_create(CAIRO_FORMAT_RGB24, 1919, 909);
...
static void open_file(GtkWidget *widget, gpointer data) {
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        char *filename = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog));
        // 불러온 이미지를 스케일링할 때도 새 해상도에 맞게 1919, 1035로 변경!
		GdkPixbuf *pixbuf = gdk_pixbuf_new_from_file_at_scale(filename, 1919, 909, FALSE, NULL);
        if (pixbuf) {
            cairo_t *cr = cairo_create(app->surface);
...
static void open_mp4(GtkWidget *widget, gpointer data) {
    Paint *app = (Paint *)data;
    GtkWidget *dialog = gtk_file_chooser_dialog_new("Open MP4", GTK_WINDOW(app->window), GTK_FILE_CHOOSER_ACTION_OPEN,
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        char *filename = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog));
        gchar *command = g_strdup_printf("cmd.exe /C start \"\" \"%s\"", filename);
        if (!g_spawn_command_line_async(command, NULL)) {
            GtkWidget *err = gtk_message_dialog_new(GTK_WINDOW(app->window), GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
                "Cannot open MP4 file.\nMake sure a default player is configured.");
...
int main(int argc, char *argv[]) {
    ...
    app->history_count = 0;
    // 메인 서피스 크기 변경
	app->surface = cairo_image_surface_create(CAIRO_FORMAT_RGB24, 1919, 909);
    clear_surface(app->surface);
    ...
    // 드로잉 영역 크기 요청 변경
	gtk_widget_set_size_request(app->drawing_area, 1919, 909);
    ...
    GtkWidget *btn_open_mp4 = gtk_button_new_with_label("동영상");
    g_signal_connect(btn_open_mp4, "clicked", G_CALLBACK(open_mp4), app);
    gtk_box_pack_start(GTK_BOX(toolbar), btn_open_mp4, FALSE, FALSE, 0);
